...
queue_types = [
    ...
    'COUNTER_PICK',                  #Nemesis (Pick oponents champs)
...
class RiotWatcher:
    ...
    def _observer_mode_request(self, url,region, **kwargs):
        if region is "":
            region = self.default_region
        args = {'api_key': self.key}
        for k in kwargs:
    ...
    def get_all_champions(self, region="na", free_to_play=False):
    ...
    # current-game-v1.0
    def get_current_game(self, summoner_id, platform_id, region):
    ...
    def get_recent_games(self, summoner_id, region):#Fix me:
